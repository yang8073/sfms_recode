...
function sale_list()
{
	...
	var now_date = null,
		...
		slider_speed = 50,
		itemListTable = new ItemListTable(slider_speed, 'sale', '.record-list'),
	...
	function init()
	{
		...
		$('.modal').on('hidden.bs.modal', function(e){
			clearTimeout(sale_item_list.modal_close_timer);
		});
	...
	function load_data(date)
	{
		...
		var url = 'api/sale/record_list/' + date,
			t_sum = 0;
		$.ajax({
			url: url,
			type: 'get',
	...
	function deleteEvent()
	{
		...
		if(!confirm('確定要刪除' + select_date + '的紀錄嗎?'))
			return;
		$.ajax({
			url: 'api/sale/delete_record/' + select_date,
			type: 'get',
			success: function(result){
				var decode_result = JSON.parse(result);
				if(!decode_result)
			...
			$.ajax({
				...
				success: function(result){
					...
					for(var key in decode_result)
					{
						...
						var $group = $('<div class="input-group sale-item-name"></div><br>'),
			...
			$.ajax({
				...
				success: function(result){
					...
					$('.input-group').animate({opacity: 0}, function(){
						...
						sale_list.modal_close_timer = setTimeout(function(){
							$('.modal').modal('hide');
						}, 2000);
					});
					...
					$.ajax({
						...
						success: function(result){
							...
							for(var key in decode_result)
							{
								...
								var $group = $('<div class="input-group sale-item-name"></div><br>').attr('data-rid', record_id),
									$addon = $('<span class="input-group-addon"></span>').text(decode_result[key]['si_name'].fill('full', item_name_max_length - decode_result[key]['si_name'].length, 'right')),
									//$record_id = $('<input type="hidden">').val(record_id),
								...
								$input_up.keyup(modal_row_clac);
								$append = $group.append($addon, $input_quantity, $input_up, $input_value);
								//append_str = '<div class="input-group sale-item-name">' +
												//'<span class="input-group-addon">' + decode_result[key]['si_name'].fill('full', item_name_max_length - decode_result[key]['si_name'].length, 'right') + '</span>' +
												//'<input type="text" class="form-control frm-quantity" data-item="' + decode_result[key]['si_id'] + '" placeholder="數量" value="' + quantity + '">' +
												//'<input type="text" class="form-control frm-up" data-item="' + decode_result[key]['si_id'] + '" placeholder="單價" value="' + up + '">' +
												//'<input type="text" class="form-control frm-cash" data-item="' + decode_result[key]['si_id'] + '" placeholder="金額" value="' + cash + '">' +
												//'</div><br>';
...
sale_list.prototype.modal_close_timer = false;

/** Stock item list page function **/
function sale_item_list()
{
	/* Var */
	var now_page,
		g_page_loading = false,
		slider_speed = 50,
		itemListTable = new ItemListTable(slider_speed, 'sale_item'),
		modal_page = '',
		modal_args = {};
	...
	function init()
	{
		...
		$('.modal').on('hidden.bs.modal', function(e){
			...
			//$('.frm-itemname').val('');
			//$('.modal-content').load('sale/additem/');
		});
		$deleteItem.click(function(){
			delete_mode(true);
		});
	...
	this.LoadData = function(page){
		...
		{
			$.ajax({
				url: 'api/sale/item_list/' + now_page,
				type: 'get',
				success: function(result){
					var decode_result = JSON.parse(result);					
		...
		if(mode)
		{
			$('.send-delete').parent('a').bind('click', function(){
				$('.slider.selected').each(function(){
					var item_id = $(this).children('div').first().text();
					$.ajax({
						url: 'api/sale/delete_item/' + item_id,
						type: 'get',
						target_row: this,
						success: function(result){
	...
	this.add_page_init = function _add(target)
	{
		...
		function submit()
		{
			...
			$.ajax({
				url: 'api/sale/add_item/' + $item_name.val(),
				type: 'get',
				success: function(result)
				{
					...
					{					
						...
						$('.input-group').animate({opacity: 0}, function(){
							...
							sale_item_list.modal_close_timer = setTimeout(function(){
								$('.modal').modal('hide');
							}, 2000);
						});
	...
	this.edit_page_init = function _edit(target)
	{
		...
		function load_data()
		{
			...
			$.ajax({
				url: 'api/sale/detail_item/' + item_id,
				type: 'get',
				success: function(result)
				{
					result = JSON.parse(result);
		...
		function submit()
		{
			...
			$.ajax({
				url: 'api/sale/update_item/' + $item_id.val() + '/' + $item_name.val(),
				type: 'get',
				success: function(result)
				{
					result = JSON.parse(result);
...
sale_item_list.prototype.modal_close_timer = false;